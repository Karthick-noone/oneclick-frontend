...
const CCTV = () => {
  ...
  // Decode the brand from URL
  const rawQuery = queryParams.get("search") || queryParams.get("brand");
  ...
  const normalizedKeywords = searchQuery
    ? searchQuery
      .toLowerCase()
      .split(/\s+/) // split by space
      .map((word) => normalizeString(word)) // normalize each word
      .filter(Boolean)
    : [];
  ...
  const filteredProducts = searchQuery
    ? products.filter((product) => {
      const combinedString = normalizeString(
        `${product.prod_name || ""} ${product.prod_features || ""} ${product.subtitle || ""}`
      );

      // Match if any keyword is included
      return normalizedKeywords.some((keyword) =>
        combinedString.includes(keyword)
      );
    })
    : products;
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (cacheRef.cctv) {
          setProducts(cacheRef.cctv);
          setCoupons(cacheRef.cctvCoupons);
          setLoading(false);
          return;
        }

        //  Fetch product list
        const response = await axios.get(`${ApiUrl}/fetchcctv`);
        const fetchedProducts = response.data;
        setProducts(fetchedProducts);
        cacheRef.cctv = fetchedProducts;
        ...
        cacheRef.cctvCoupons = couponMap;
      } catch (error) {
        console.error("Error fetching cctv:", error);
        toast.error("Failed to fetch cctv.", {
          position: "top-right",
          autoClose: 2000,
