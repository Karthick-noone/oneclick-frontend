...
const EditHomePageImageAd = () => {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      await axios.put(
        `${ApiUrl}/updateedithomepage/${editingProduct.id}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  const handleDeleteProduct = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`${ApiUrl}/api/deleteedithomepage/${editingProduct.id}`, {
            method: 'DELETE',
          });
  ...
  const handleDeleteImage = async (product, imageIndex) => {
    ...
    if (confirmResult.isConfirmed) {
      try {
        ...
        await axios.put(`${ApiUrl}/deleteedithomepageimage/${product.id}`, {
  ...
  const handleUpdateImage = async () => {
    ...
    try {
      const response = await axios.put(
        `${ApiUrl}/updateedithomepageimage/${editingProduct.id}/${editingImageIndex}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
...
return (
  <div className="laptops-page">
    <div className="laptops-content">
      <h2 className="laptops-page-title">Edit Home Ad Page</h2>
      <div className="laptops-card">
        ...
        <div className="laptops-card-row">
          ...
          <FaInfoCircle  style={{cursor:'pointer',fontSize:'18px'}} title="Add potrait images for better view (2000 x 600)" />
        </div>
      </div>
...
export default EditHomePageImageAd;
