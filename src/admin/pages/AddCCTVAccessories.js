...
const CCTVAccessories = () => {
  ...
  const handleImageUpdate = () => {
    ...
    if (selectedFile) {
      ...
      fetch(
        `${ApiUrl}/updatecctvaccessories/image/${productId}?index=${imageIndex}`,
        {
          method: "PUT",
          body: formData,
        }
  ...
  const handleDeleteImage = (productId, imageIndex) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(
          `${ApiUrl}/deletecctvaccessories/image/${productId}?index=${imageIndex}`,
          {
            method: "DELETE",
          }
        )
  ...
  if (!hasDigit) {
    ...
    try {
      ...
      // Fetch updated list of products
      const fetchResponse = await axios.get(`${ApiUrl}/fetchcctvaccessories`);
    ...
    if (confirmResult.isConfirmed) {
      try {
        ...
        console.log(
          ...
          `${ApiUrl}/deletecctvaccessories/${id}`
        );
  ...
  return (
    <div className="laptops-page">
      <div className="laptops-content">
        <h2 className="laptops-page-title">Add CCTVaccessories</h2>
