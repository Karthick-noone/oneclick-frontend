...
const Speakers = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`${ApiUrl}/fetchspeakers`);
  ...
  if (!editingProduct.price || isNaN(editingProduct.price) || Number(editingProduct.price) <= 0) {
    ...
    try {
      // Log the request URL
      console.log('Sending update request to:', `${ApiUrl}/updatespeakers/${editingProduct.id}`);
  
      // Send the update request
      const response = await axios.put(`${ApiUrl}/updatespeakers/${editingProduct.id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
      ...
      // Fetch updated list of products
      const fetchResponse = await axios.get(`${ApiUrl}/fetchspeakers`);
    ...
    <div className="laptops-page">
      <div className="laptops-content">
        ...
        <div className="laptops-product-image">
          <img
            src={`${ApiUrl}/uploads/speakers/${product.prod_img}`} 
