...
const Laptops = () => {
  ...
  if (!editingProduct.price || isNaN(editingProduct.price) || Number(editingProduct.price) <= 0) {
    ...
    try {
      // Log the request URL
      console.log('Sending update request to:', `${ApiUrl}/updatecomputers/${editingProduct.id}`);
  
      // Send the update request
      const response = await axios.put(`${ApiUrl}/updatecomputers/${editingProduct.id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
  ...
  const handleDeleteProduct = async (id) => {
    ...
    if (confirmResult.isConfirmed) {
      try {
        ...
        console.log('Sending delete request to:', `${ApiUrl}/deletecomputers/${id}`);
        
        // Perform the delete operation
        const deleteResponse = await axios.delete(`${ApiUrl}/deletecomputers/${id}`);
    ...
    <div className="laptops-page">
      <div className="laptops-content">
        ...
        <div className="laptops-product-image">
          <img
            src={`${ApiUrl}/uploads/computers/${product.prod_img}`} 
