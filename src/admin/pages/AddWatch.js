...
const Watch = () => {
  ...
  const handleUploadImages = async (productId) => {
    ...
    try {
      const response = await fetch(`${ApiUrl}/uploadwatchimages`, {
        method: "POST",
        body: formData,
      });
  ...
  const handleImageUpdate = () => {
    ...
    if (selectedFile) {
      ...
      fetch(`${ApiUrl}/updatewatch/image/${productId}?index=${imageIndex}`, {
        method: "PUT",
        body: formData,
      })
  ...
  const handleDeleteImage = (productId, imageIndex) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`${ApiUrl}/deletewatch/image/${productId}?index=${imageIndex}`, {
          method: "DELETE",
        })
          .then((response) => response.json())
          .then((data) => {
  ...
  const handleAddProduct = async () => {
    // Check if label exceeds 30 characters
    ...
    // Ensure newProduct.images is defined and is an array before checking length
    ...
    // Validate coupon code and expiry date
    const couponCode = newProduct.coupon;
    const couponExpiryDate = newProduct.coupon_expiry_date;
    ...
    // Ensure coupon code is not just spaces
    ...
    const couponValueMatch = couponCode ? couponCode.match(/\d+/) : null;
    const couponValue = couponValueMatch ? Number(couponValueMatch[0]) : 0;
  
    // Ensure coupon value is less than product price
    ...
    try {
      await axios.post(`${ApiUrl}/watch`, formData, {
        headers: {
  ...
  if (!hasDigit) {
    ...
    try {
      ...
      console.log(
        ...
        `${ApiUrl}/updatewatch/${editingProduct.id}`
      );
      ...
      // Fetch updated list of products
      const fetchResponse = await axios.get(`${ApiUrl}/fetchwatch`);
