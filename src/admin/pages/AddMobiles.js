...
const AddMobiles = () => {
  ...
  if (!editingProduct.price || isNaN(editingProduct.price) || Number(editingProduct.price) <= 0) {
    ...
    try {
      // Log the request URL
      console.log('Sending update request to:', `${ApiUrl}/updatemobiles/${editingProduct.id}`);
  
      // Send the update request
      const response = await axios.put(`${ApiUrl}/updatemobiles/${editingProduct.id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
  ...
  const handleDeleteProduct = async (id) => {
    ...
    if (confirmResult.isConfirmed) {
      try {
        ...
        console.log('Sending delete request to:', `${ApiUrl}/deletemobiles/${id}`);
        
        // Perform the delete operation
        const deleteResponse = await axios.delete(`${ApiUrl}/deletemobiles/${id}`);
    ...
    <div className="laptops-page">
      <div className="laptops-content">
        ...
        <div className="laptops-product-image">
          <img
            src={`${ApiUrl}/uploads/mobiles/${product.prod_img}`} 
