...
const CCTV = () => {
  ...
  const handleImageUpdate = () => {
    ...
    if (selectedFile) {
      ...
      fetch(
        `${ApiUrl}/updatecctv/image/${productId}?index=${imageIndex}`,
        {
          method: "PUT",
          body: formData,
        }
  ...
  const handleDeleteImage = (productId, imageIndex) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(
          `${ApiUrl}/deletecctv/image/${productId}?index=${imageIndex}`,
          {
            method: "DELETE",
          }
        )
  ...
  const couponExpiryDate = newProduct.coupon_expiry_date; // Assuming expiry date is stored here
   ...
   if ((couponCode && !couponExpiryDate) || (!couponCode && couponExpiryDate)) {
    Swal.fire({
      icon: "warning",
      title: "Validation Error",
 ...
 editingProduct.coupon_expiry_date !== "0000-00-00"
  ...
  if (couponValue >= Number(editingProduct.price)) {
    ...
    try {
      ...
      // Fetch updated list of products
      const fetchResponse = await axios.get(`${ApiUrl}/fetchcctv`);
    ...
    <div className="laptops-page">
      <div className="laptops-content">
        ...
        <div className="laptops-card">
          ...
          <div className="feature-1 laptops-card-item ">Coupon Expiry date</div>
