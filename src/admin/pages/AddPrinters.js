...
import { FaEdit, FaTrash, FaEye, FaTimes, FaClone, FaImages } from "react-icons/fa"; // Import icons
import Modal from "react-modal";
import Swal from "sweetalert2";
import { useNavigate } from "react-router-dom";
import Slider from "react-slick"; // Import Slider from react-slick
import { FaInfoCircle, FaPlusCircle, FaChevronDown } from "react-icons/fa"; // Ensure to import any icons you need
...
const Printers = () => {
  ...
  const handleUploadImages = async (productId) => {
    ...
    try {
      const response = await fetch(`${ApiUrl}/uploadprintersimages`, {
        method: "POST",
        body: formData,
      });
  ...
  const handleImageUpdate = () => {
    ...
    if (selectedFile) {
      ...
      fetch(`${ApiUrl}/updateprinters/image/${productId}?index=${imageIndex}`, {
        method: "PUT",
        body: formData,
      })
  ...
  const handleAddProduct = async () => {
    ...
    try {
      await axios.post(`${ApiUrl}/printers`, formData, {
        headers: {
  ...
  const handleUpdateProduct = async () => {
    ...
    console.log(
      ...
      `${ApiUrl}/updateprinters/${editingProduct.id}`
    );
  ...
  const handleAddAccessories = () => {
    navigate("/Admin/printersAccessories");
  };
  ...
  return (
    <div className="laptops-page">
      <div className="laptops-content">
        <h2 className="laptops-page-title">Add Printers</h2>
        ...
        <h2 className="laptops-page-title">Printer Product List</h2>
        ...
        <div className="laptops-products-list">
          {products.length === 0 ? (
            <div className="empty-state-message"><FaInfoCircle /> No products available. Please add some Printer products.</div>
          ) : (
            (() => {
                          ...
                          >
                            {product.prod_img.map((img, imgIndex) => (
                              <div key={imgIndex} className="image-wrapper">
                                <img
                                  src={`${ApiUrl}/uploads/printers/${img}`}
                                  alt={product.prod_name}
                                  className="laptops-product-image"
                                  onClick={() =>
                                    handleImageClick(
                                      imgIndex, // Index of the clicked image in this product
                                      product.prod_img.map(img => `${ApiUrl}/uploads/printers/${img}`) // Only current product's images
                                    )
                                  }
                                />
