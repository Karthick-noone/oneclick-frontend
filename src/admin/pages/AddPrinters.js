...
const Printers = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`${ApiUrl}/fetchprinters`);
  ...
  if (!editingProduct.price || isNaN(editingProduct.price) || Number(editingProduct.price) <= 0) {
    ...
    try {
      // Log the request URL
      console.log('Sending update request to:', `${ApiUrl}/updateprinters/${editingProduct.id}`);
  
      // Send the update request
      const response = await axios.put(`${ApiUrl}/updateprinters/${editingProduct.id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
      ...
      // Fetch updated list of products
      const fetchResponse = await axios.get(`${ApiUrl}/fetchprinters`);
    ...
    <div className="laptops-page">
      <div className="laptops-content">
        ...
        <div className="laptops-product-image">
          <img
            src={`${ApiUrl}/uploads/printers/${product.prod_img}`} 
