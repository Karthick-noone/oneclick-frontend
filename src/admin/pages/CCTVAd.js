...
const CCTVAd = () => {
 ...
 useEffect(() => {
  ...
  const handleAddProduct = async () => {
    ...
    try {
      await axios.post(`${ApiUrl}/cctvofferspage`, formData, {
        headers: {
      ...
      Swal.fire({
        ...
        return axios.get(`${ApiUrl}/fetchcctvofferspage`);
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      await axios.put(
        `${ApiUrl}/cctvupdateofferspage/${editingProduct.id}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  const handleDeleteProduct = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`${ApiUrl}/api/deletecctvofferspage/${editingProduct.id}`, {
            method: 'DELETE',
          });
  ...
  const handleDeleteImage = async (product, imageIndex) => {
    ...
    if (confirmResult.isConfirmed) {
      try {
        ...
        await axios.put(`${ApiUrl}/deletecctvofferspageimage/${product.id}`, {
  ...
  const handleUpdateImage = async () => {
    ...
    try {
      const response = await axios.put(
        `${ApiUrl}/updatecctvofferspageimage/${editingProduct.id}`, // Update the endpoint to only include the product ID
        formData,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
...
return (
  <div className="laptops-page">
    <div className="laptops-content">
      <h2 className="laptops-page-title">Edit CCTV Offers</h2>
            ...
            {product.image && product.image.length > 0 ? (
              <div className="product-image-wrapper"> {/* Wrapper for positioning */}
                ...
                <img
                  ...
                  // style={{ width: '100%', height: 'auto' }} // Set size as specified
