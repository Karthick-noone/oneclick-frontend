  ...
  const handleAddProduct = async () => {
    ...
    try {
      ...
      Swal.fire({
        ...
        timer:3000,
      }).then(() => axios.get(`${ApiUrl}/fetchcctvofferspage`))
 ...
 const handleUpdateProduct = async () => {
  ...
  try {
    ...
    if (selectedFiles) {
      ...
      // Update the image
      await axios.put(
        `${ApiUrl}/updatecctvofferspageimage/${editingProduct.id}`,
        imageFormData,
        {
          headers: {
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(
            `${ApiUrl}/api/deletecctvofferspage/${editingProduct.id}`,
            {
              method: "DELETE",
            }
          );
  ...
  const handleDeleteImage = async (product, imageIndex) => {
    ...
    if (confirmResult.isConfirmed) {
      try {
        ...
        await axios.put(
          `${ApiUrl}/deletecctvofferspageimage/${product.id}`,
          {
