...
const AddComputerAccessories = () => {
  ...
  if (!editingProduct.price || isNaN(editingProduct.price) || Number(editingProduct.price) <= 0) {
    ...
    try {
      // Log the request URL
      console.log('Sending update request to:', `${ApiUrl}/updatecomputeraccessories/${editingProduct.id}`);
  
      // Send the update request
      const response = await axios.put(`${ApiUrl}/updatecomputeraccessories/${editingProduct.id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
      ...
      // Fetch updated list of products
      const fetchResponse = await axios.get(`${ApiUrl}/fetchcomputeraccessories`);
    ...
    <div className="laptops-page">
      <div className="laptops-content">
        ...
        <div className="laptops-product-image">
          <img
            src={`${ApiUrl}/uploads/computeraccessories/${product.prod_img}`} 
...
export default AddComputerAccessories;